use super::{
    ...
    path_argument, unexpected, Cli, CommandSpec, CHECKPOINT_OPTION, FILE_OPTION, HELP_OPTION,
    VERSION_OPTION,
};
use crate::Result;
use lexopt::Parser;
...
pub(crate) static SPEC: CommandSpec = CommandSpec {
    name: "restore",
    about: "Restore a retained checkpoint for the same authored world.",
    options: &[FILE_OPTION, CHECKPOINT_OPTION],
    positionals: &[],
    examples: &["smolworld restore --checkpoint /absolute/path/checkpoint"],
    subcommands: &[],
};
...
pub(crate) fn parse(parser: &mut Parser, mut config: PathBuf) -> Result<Cli> {
        ...
        .map_err(|error| parse_error(SPEC.name, error))?
    {
        match arg {
            ...
            arg if option_matches(&arg, &CHECKPOINT_OPTION) && checkpoint.is_none() => {
                checkpoint = Some(path_argument(parse_value(
                    parser,
                    SPEC.name,
                    &CHECKPOINT_OPTION,
                )?));
            ...
            arg if option_matches(&arg, &CHECKPOINT_OPTION) => {
                return Err(format!(
                    ...
                    SPEC.name,
                    option_display(&CHECKPOINT_OPTION)
                ))
            }
    ...
    Ok(Cli::Restore { config, checkpoint })
}
