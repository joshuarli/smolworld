...
pub(crate) static SPEC: CommandSpec = CommandSpec {
    ...
    options: &[FILE_OPTION, OUTPUT_OPTION, PARENT_OPTION],
    positionals: &[],
...
pub(crate) fn parse(parser: &mut Parser, mut config: PathBuf) -> Result<Cli> {
        ...
        .map_err(|error| parse_error(SPEC.name, error))?
    {
        match arg {
            ...
            arg if option_matches(&arg, &PARENT_OPTION) && parent.is_none() => {
                parent = Some(path_argument(parse_value(
                    parser,
                    SPEC.name,
                    &PARENT_OPTION,
                )?));
            }
            arg if option_matches(&arg, &PARENT_OPTION) => {
                return Err(format!(
                    "{} accepts {} at most once",
                    SPEC.name,
                    option_display(&PARENT_OPTION)
                ))
            }
    ...
    Ok(Cli::Checkpoint { config, output, parent })
}
