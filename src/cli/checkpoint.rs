use super::{command_help, missing, option_display, option_matches, parse_error, parse_file, parse_value, path_argument, unexpected, Cli, CommandSpec, FILE_OPTION, HELP_OPTION, OUTPUT_OPTION, VERSION_OPTION};
use crate::Result;
use lexopt::Parser;
    ...
    while let Some(arg) = parser.next().map_err(|error| parse_error(SPEC.name, error))? {
        match arg {
            ...
            arg if option_matches(&arg, &OUTPUT_OPTION) && output.is_none() => {
                output = Some(path_argument(parse_value(parser, SPEC.name, &OUTPUT_OPTION)?));
            }
            arg if option_matches(&arg, &OUTPUT_OPTION) => {
                return Err(format!("{} accepts {} at most once", SPEC.name, option_display(&OUTPUT_OPTION)))
            }
